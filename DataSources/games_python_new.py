ddef generate_quran_game_html_with_extra(arabic_text, surah_name, surah_name_ar, ayah_num, english_text, word_data, audio_url, transliterations, urdu_text, urdu_audio, english_audio, required_keys, jarr_phrases=None, output_file="ayah_memorize.html"):
...
<!-- âœ… Firebase Compat SDKs for static HTML -->
...
""")
    ...
    for word in word_data:
        filtered_word = {k: word[k] for k in req_keys if k in word}
        morph_json = dumps(filtered_word, ensure_ascii=False).replace('"', '&quot;')

        case = (word.get("Case_from_xml") or "").strip()
        case_class = case_map.get(case.upper(), "") if case else ""
        verb_class = "verb-highlight" if word.get("verb_colour") else ""

        append(f"""
    <span class="word-block click-animate {case_class} {verb_class}" onclick="showPopup({morph_json})">

    <!-- Arabic word only -->
    <span class="word-text">{escape(word.get('Word', ''))}</span>

    <!-- Metadata: translation, root, POS -->
    <span class="metadata">
        <span class="translation toggle-translation">{escape(word.get('Translation', 'â€”'))}</span>
        <span class="translation toggle-root root-tag ">{escape(word.get('root_from_txt', ''))}</span>
        <span class="translation toggle-pos pos-tag ">{escape(word.get('tags_simple_joined', '').lower())}</span>
    </span>
...
<div class="grammar-info">
  <div style="display: flex; justify-content: center; flex-wrap: wrap; row-gap: 10px; column-gap: 20px; margin-top: 12px;">
    ...
    <div class="grammar-item" onclick="showGrammarPopup('nominative')"
      ...
      <span><b>Nominative</b> <i>(RafÊ¿ / Subject)</i></span>
    </div>
    ...
    <div class="grammar-item" onclick="showGrammarPopup('accusative')"
      ...
      <span><b>Accusative</b> <i>(Naá¹£b / Object)</i></span>
    </div>
    ...
    <div class="grammar-item" onclick="showGrammarPopup('verb')"
      ...
      <span><b>Verb</b> <i>(FiÊ¿l / Action)</i></span>
    </div>
...
<button id="stripNavGames" class="strip-btn">
  <span class="btn-text">Play and Practise â†’</span>
</button>
...
""")
    ...
    append(f"""
            <div id="transliteration-box" class="section" style="display:none; text-align:left">
                 <div class="section practice-header">
                    <h2 class="practice-title">âœï¸ Practice Mode: Write the Arabic Words</h2>
                    ...
                    <p class="practice-desc">
                       (Use the <strong>Settings</strong> (âš™ï¸) to clear your inputs or toggle translations on/off).
                    </p>
                 </div>
    ...
    for idx, word in enumerate(word_data):
        trans = escape(word.get("Translation", "â€”"))
        translit = escape(word.get("translit", ""))
...
""")
    ...
    print(f"âœ… HTML file generated: {filepath}")
v id="wordHintOverlay" class="word-hint-overlay hidden">
    <div class="word-hint-text">
    Click on each word to view details
    </div>
    <div id="wordHint" class="word-hint">
